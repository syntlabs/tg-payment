...
from config import PERCENTAGE_OF_REFERRALS
...
async def handle_get_referral_link_cbd(
    ...
    await callback_query.message.answer(
        MESSAGES["your_referral_link_text"][language]
        .format(link, link.split("start=")[-1], PERCENTAGE_OF_REFERRALS)
    )
